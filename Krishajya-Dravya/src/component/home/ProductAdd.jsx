...
const ProductAdd = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 pb-20">
        {products.map((item) => (
          ...
          >
            ...
            <p className="text-gray-500 text-sm text-center mt-1">{item.description}</p>
      ...
      {/* View Cart Button (Sticky at bottom, only if cart has items) */}
      {Object.keys(cart).length > 0 && (
        <div className="fixed bottom-5 left-1/2 transform -translate-x-1/2">
          <button className="bg-pink-600 hover:bg-pink-700 text-white px-10 py-3 rounded-full shadow-lg text-lg font-bold transition-all duration-300">
